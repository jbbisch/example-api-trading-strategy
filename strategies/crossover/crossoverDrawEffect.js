...
const drawEffect = (state, action) => {
    ...
    if(event === 'crossover/draw') {
        ...
        const { atrHourState, PTarmCount, product, position, mode, buffer, tlc, realizedPnL, buyDistance, sellDistance, buyTriggerSource, sellTriggerSource,} = state
        ...
        drawToConsole({
            ...
            mode,
            ATR_rank: atrHourState?.rank != null ? `${atrHourState.rank.toFixed(0)}%ile${atrHourState.rank >= 80 ? ' PASS' : ' (need 80)'}` : 'warming up',
