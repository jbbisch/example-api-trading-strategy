  ...
  if (!this.isConnected()) {
    ...
    this._reconnectTimer = setTimeout(async () => {
      ...
      try {
        ...
        // Save current buffer and ATR state before disconnecting
        const oldBuffer   = this.strategy?.state?.buffer?.getData() || []
        const oldAtrState = this.strategy?.state?.atrHourState || null
        ...
        if (this.strategy) {
          ...
          if (oldAtrState) state.atrHourState = oldAtrState
